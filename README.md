...
```text
NewsRSSHub
最近 6 小时有 X 条未读新内容，点此查看
```

其中 `X` 是最近 6 小时内、首页仍可见且尚未主动阅读的合并新闻数；不会统计已隐藏内容，也不会把原始抓取条目直接相加。只有本轮确实抓到新内容、完成摘要和筛选，且该数大于 0 时才会发送。点击通知始终打开首页。内容完成筛选后会等待 1 分钟收拢错峰来源，之后在当前全局抓取间隔内最多再发一条。临时网络错误会重试；浏览器返回订阅失效时，系统会停止推送并要求重新开启。
