...
> 当前实现补充（以本段为准）：数据库目标结构已升级至 v10。v9 → v10 仅新增 `weekly_topics` 和 `weekly_topic_events` 两张表，不修改既有 `items`、`events`、`briefs` 或来源数据；但为避免 Web 与 Worker 并发改表，仍必须停止服务后通过维护命令显式执行预检、备份和迁移。
>
> 下文记录的 v7 / v8 结构收敛机制保留为历史背景。v8 及更早版本会在同一次显式维护中直接重建到 v10；不会先在运行时自动升到中间版本。
>
> 状态：已批准实施
>
> 适用版本：数据库结构 v10（v8 结构收敛说明保留为历史背景）
>
...
本次保留已经实现的产品能力：来源管理、平台凭证、X Cookie 动态维护、LLM 连接配置、内容摘要与高亮、正文中文翻译、事件合并、四个内容 Tab、已读、不感兴趣、本周热点、历史日报与批量来源导入。
...
| `weekly_topics` | 本周话题身份与展示名 | `(week_start, id)` 唯一；`id` 是稳定身份，`display_name` 可随本周进展更新 |
| `weekly_topic_events` | 本周话题到事件的归属 | 主键 `(week_start, event_id)`，保证同一事件一周内只属于一个话题 |
...
- `weekly_topics.id`：话题的稳定数据库身份；模型只能引用本周已提供的 ID，名称改变不会改变内容组织关系。
- `weekly_topic_events`：只保存“事件属于哪个本周话题”，不复制内容计数；页面每次按周窗口实时统计可见内容、事件和来源数。
...
- `weekly_topics` 与 `weekly_topic_events`：仅服务短期浏览，超过 30 天随话题表清理；关联事件仍由普通保留策略决定。
- 收藏的事件及其关联内容：不自动清理。
...
首次迁移时，先在 Portainer 部署包含 v10 迁移的最新提交，让新镜像构建完成；旧结构数据库会让 Web/Worker 拒绝启动，但不会修改数据。随后在服务器终端使用这次构建出的镜像执行维护命令。例如镜像名为 `newsrsshub-migrate:20260805` 时：
...
2. 再次执行预检，失败即中止。
3. 对完整 v9 库，在同一事务中只创建周话题两表及索引，并写入 `PRAGMA user_version = 10`；既有业务表不重建、不复制。
4. 对 v8 及更早库，在同一事务中重建目标表并按主键复制保留数据；旧日报仅移除指向不存在事件的 ID，保留日报行、其余有效 ID 及原有顺序，再写入 `PRAGMA user_version = 10`。
5. 迁移后复核行数、外键、事件关系、周话题表、X 配置缓存与加密凭证记录。
6. 失败时回滚数据库事务；已创建的备份保留用于人工恢复。

普通应用启动仅允许两种情况：空数据库初始化为 v10，或数据库已经是完整 v10。遇到旧版本会拒绝启动并提示先运行迁移命令，避免在 Web 服务启动过程中悄悄做不可逆重建。
...
5. `collector`、`processor` 与 `topics` Worker 能正常运行；本周热点页只显示周一零点至今累计至少 2 条的可见内容统计。

如需回滚：停止服务，使用迁移命令输出的备份恢复 `rss_news.db`，再部署旧版镜像。不要在仍运行的 WAL 数据库上直接覆盖单个 `.db` 文件。

## 8. 本次代码落地范围

- `Database.initialize()` 只接受空库或完整 v10；旧库会给出迁移提示，不会在 Web/Worker 启动时改表。
- `app.migrate` 提供 `--check`（SQLite `mode=ro` 只读连接）与 `--apply`（backup API 备份、单事务迁移、迁移后校验）。命令输出只含版本和行数，不输出任何凭证。
- 对旧库日报中仅有的失效事件引用，`--check` 会明确显示可自动移除数量，`--apply` 才会在备份后的事务中修复；无法解析的日报 JSON 仍会拒绝迁移。已是完整 v10 的数据库不会被普通运行时或迁移命令静默改写。
...
- 自动化测试覆盖：空库初始化、旧库拒绝普通启动、只读预检、备份后的显式迁移、关系不一致阻断、v9 → v10 加法迁移，以及 v10 外键/索引/完整性检查。
