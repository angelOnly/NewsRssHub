...
class WorkerTests(unittest.TestCase):
    def _services(self) -> Mock:
        ...
        services.pipeline.refresh_daily_topics_once.return_value = {
            "daily_topics": {"refreshed": True}
        }
        return services
    ...
    def test_topics_role_only_runs_the_dedicated_topic_pipeline(self) -> None:
        ...
        services.pipeline.refresh_daily_topics_once.assert_called_once_with()
        services.pipeline.process_once.assert_not_called()

    def test_topics_role_uses_the_persisted_minute_interval(self) -> None:
        services = self._services()
        services.repository.get_daily_topic_refresh_interval_minutes.return_value = 45
        with (
